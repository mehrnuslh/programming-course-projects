...
def index():
    ...
    transactions_db = db.execute(
        "SELECT symbol, SUM(shares) as shares, price, total FROM transactions WHERE user_id = ? GROUP BY symbol",
        user_id,
    )
    ...
    for stock in transactions_db:
        ...
        stock["totalprice"] = stock_data["price"] * stock["shares"]
        current_worth += stock["totalprice"]

    return render_template(
        "index.html", database=transactions_db, cash=cash, current_worth=current_worth
    )
...
def sell():
    ...
    if request.method == "GET":
        ...
        total = user_shares_real - shares
        total_value = total * stock["price"]
